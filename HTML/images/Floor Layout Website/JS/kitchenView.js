		   ...
		   document.addEventListener("DOMContentLoaded", async () => 
		   {
			...
			async function getLights()
			{
				try
				{
					...
					data.forEach(room =>
					{
						const lightButton = document.getElementById(`${room.roomName.toLowerCase().replace(/\s+/g, '-')}-light`);
						if (lightButton)
						{
					...
					{
						const kitchenButton = document.getElementById(`kitchen-${type}`);
						if (kitchenButton)
						{
							kitchenButton.classList.toggle('green', kitchenData[0][type]);
							kitchenButton.classList.toggle('red', !kitchenData[0][type]);
						}
					});
				}	
				catch (error)
			...
			async function toggleKitchenLights(lightType)
			{
			const lightButton = document.getElementById(`kitchen-${lightType}`);
				...
				try
				{
					const response = await fetch(`/rooms/kitchen/lights`,
					{
						method: "PUT",
						headers: {"Content-Type": "application/json"},
						body: JSON.stringify({lightType, state: newState})
					});
					...
					{
						console.error("Failed to update the kitchen light states!");
					}
				}
				catch (error)
				{
					console.error("Error toggling the kitchen lights!", error);
				}
			}
